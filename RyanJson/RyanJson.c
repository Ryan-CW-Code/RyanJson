...
#define parseSkipInvalidChar(buf, errCode)                                                                                                                                                             \
	{                                                                                                                                                                                              \
		while (*(buf)->address && (*(buf)->address == ' ' || *(buf)->address == '\n' || *(buf)->address == '\r'))                                                                              \
		{                                                                                                                                                                                      \
...
#define printBufPuts(buf, s, len)                                                                                                                                                                      \
	{                                                                                                                                                                                              \
		for (int32_t i = 0; i < (len); i++) printBufPutc(buf, (s)[i]);                                                                                                                         \
	}
...
typedef struct
{
	RyanJsonBool_e noalloc; // 是否动态申请内存
...
static RyanJsonBool_e RyanJsonParseValue(parseBuffer *buf, char *key, RyanJson_t *out);
static RyanJsonBool_e RyanJsonPrintValue(RyanJson_t pJson, printBuffer *buf, uint32_t depth, RyanJsonBool_e format);

static void *tempRealloc(void *block, size_t size)
{
 ...
 */
RyanJsonBool_e RyanJsonInitHooks(malloc_t userMalloc, free_t userFree, realloc_t userRealloc)
{
	...
	jsonFree = userFree;
	jsonRealloc = NULL != userRealloc ? userRealloc : tempRealloc;
 ...
 * @return RyanJsonBool_e
 */
static RyanJsonBool_e expansion(printBuffer *buf, uint32_t needed)
{
	char *address = NULL;
	uint32_t size = 0;
...
{
	int32_t size = RyanMqttStrlen(str) + 1;
 ...
 */
static RyanJson_t RyanJsonNewNode(uint32_t info, char *key)
{
...
static uint32_t RyanJsonParseHex(const char *text)
{
	...
	for (uint8_t i = 0; i < 4; ++i)
	{
		...
		case '7':
		case '8':
		case '9': value = (value << 4) + (text[i] - '0'); break;
		case 'a':
		case 'b':
		case 'c':
		case 'd':
		case 'e':
		case 'f': value = (value << 4) + 10 + (text[i] - 'a'); break;
		case 'A':
		case 'B':
		case 'C':
		case 'D':
 ...
 */
static RyanJsonBool_e RyanJsonParseNumber(parseBuffer *buf, char *key, RyanJson_t *out)
{
 ...
 * @return RyanJsonBool_e
 */
static RyanJsonBool_e parse_string_buffer(parseBuffer *buf, char **buffer)
{
	char *p2 = NULL;
	char *out = NULL;
	int32_t len = 0;
	...
	*buffer = NULL;
	...
	while (*buf->address && *buf->address != '\"')
	{
		...
		case 'u': {
			...
			{
				p2[utf8_position] = (char)((uc | 0x80) & 0xBF); // 10xxxxxx
				uc >>= 6;
			}
			...
			if (utf8_length > 1)
			{
				p2[0] = (char)((uc | first_byte_mark) & 0xFF);
			}
			else
			{
				p2[0] = (char)(uc & 0x7F);
 ...
 * @return const char*
 */
static RyanJsonBool_e RyanJsonParseString(parseBuffer *buf, char *key, RyanJson_t *out)
{
	RyanJson_t newItem = NULL;
	char *buffer = NULL;
 ...
 * @return const char*
 */
static RyanJsonBool_e RyanJsonParseArray(parseBuffer *buf, char *key, RyanJson_t *out)
{
 ...
 * @return const char*
 */
static RyanJsonBool_e RyanJsonParseObject(parseBuffer *buf, char *key, RyanJson_t *out)
{
	RyanJson_t newItem = NULL, prev = NULL, item = NULL;
	char *k = NULL;
 ...
 * @return RyanJsonBool_e
 */
static RyanJsonBool_e RyanJsonPrintNumber(RyanJson_t pJson, printBuffer *buf)
{
	double f;
	int32_t len = 0;
	...
	if (_checkType(pJson->info, RyanJsonValueNumberIntFlag))
	{
		...
		len = RyanMqttSprintf(printBufEnd(buf), "%" PRId32, RyanJsonGetIntValue(pJson));
		buf->end += len;
	}
	else // RyanJsonNumber 的类型是浮点型
	{
		...
		{
			len = RyanMqttSprintf(printBufEnd(buf), "%.1lf", f);
		}

		// use exponential form conversion beyond the limited range
		else if (fabs(f) < 1.0e-6 || fabs(f) > 1.0e9)
		{
			len = RyanMqttSprintf(printBufEnd(buf), "%e", f);
		}

		// default conversion
		else
		{
			len = RyanMqttSprintf(printBufEnd(buf), "%lf", f);
 ...
 * @return RyanJsonBool_e
 */
static RyanJsonBool_e print_string_buffer(const char *str, printBuffer *buf)
{
	const char *p = NULL;
	int32_t len = 0;
	RyanJsonBool_e escape = RyanJsonFalse;
	...
	while (*p)
	{
		...
		case '\r':
		case '\t':
		case '/': printBufPutc(buf, *p); break;

		default:
			RyanMqttSprintf(printBufEnd(buf), "u%04x", (uint8_t)(*p));
			buf->end += 5; // utf
			break;
		}
		p++;
...
static inline RyanJsonBool_e RyanJsonPrintString(RyanJson_t pJson, printBuffer *buf)
{
 ...
 * @return RyanJsonBool_e
 */
static RyanJsonBool_e RyanJsonPrintArray(RyanJson_t pJson, printBuffer *buf, uint32_t depth, RyanJsonBool_e format)
{
	int32_t count = 0;
	...
	if (format)
	{
		...
		{
			if ((_checkType(child->info, RyanJsonTypeArray) || _checkType(child->info, RyanJsonTypeObject)) && RyanJsonGetObjectValue(child))
			{
				count++;
				break;
			}
 ...
 * @return RyanJsonBool_e
 */
static RyanJsonBool_e RyanJsonPrintObject(RyanJson_t pJson, printBuffer *buf, uint32_t depth, RyanJsonBool_e format)
{
	RyanJson_t child = RyanJsonGetObjectValue(pJson);
...
static RyanJsonBool_e RyanJsonPrintValue(RyanJson_t pJson, printBuffer *buf, uint32_t depth, RyanJsonBool_e format)
{
	...
	case RyanJsonTypeNumber: return RyanJsonPrintNumber(pJson, buf);
	case RyanJsonTypeString: return RyanJsonPrintString(pJson, buf);
	case RyanJsonTypeArray: return RyanJsonPrintArray(pJson, buf, depth, format);
	case RyanJsonTypeObject: return RyanJsonPrintObject(pJson, buf, depth, format);
	}
 ...
 *
 * @param text 文本地址
 * @param mustBeNullTerminated 是否允许解析的文本后面有无意义的字符
 * @param parseEndPtr 输出解析终止的字符位置
 * @return RyanJson_t
 */
RyanJson_t RyanJsonParseOptions(const char *text, uint32_t size, RyanJsonBool_e mustBeNullTerminated, const char **parseEndPtr)
{
	RyanJson_t pJson = NULL;
	parseBuffer buf = {0};
 ...
 * @return RyanJsonBool_e
 */
RyanJsonBool_e RyanJsonReapplyString(char **dst, const char *src)
{
	char *k = NULL;
...
RyanJson_t RyanJsonGetObjectByKey(RyanJson_t pJson, const char *key)
{
	...
	while (nextItem && 0 != RyanMqttStrcmp(RyanJsonGetKey(nextItem), key))
	{
		nextItem = nextItem->next;
	}
...
RyanJsonBool_e RyanJsonInsert(RyanJson_t pJson, int32_t index, RyanJson_t item)
{
	...
	if (!(_checkType(pJson->info, RyanJsonTypeArray)) && !(_checkType(pJson->info, RyanJsonTypeObject) && (item->info & RyanJsonWithKeyFlag)))
	{
...
RyanJsonBool_e RyanJsonReplaceByKey(RyanJson_t pJson, const char *key, RyanJson_t item)
{
	...
	else if (0 != RyanMqttStrcmp(RyanJsonGetKey(item), key))
	{
		// 修改key
 ...
 * @return RyanJson_t
 */
RyanJson_t RyanJsonCreateBool(char *key, RyanJsonBool_e boolean)
{
	RyanJson_t item = NULL;
	uint32_t info = RyanJsonTypeBool;
	char *k = NULL;
...
RyanJsonBool_e RyanJsonCompare(RyanJson_t a, RyanJson_t b)
{
	...
	case RyanJsonTypeString: return 0 == RyanMqttStrcmp(RyanJsonGetStringValue(a), RyanJsonGetStringValue(b)) ? RyanJsonTrue : RyanJsonFalse;
