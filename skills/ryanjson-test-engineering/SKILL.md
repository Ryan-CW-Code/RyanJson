...
description: RyanJson 单元测试工程与结构重构：补单测、去重审查、用例归属调整与 test-map 更新；仅在用户明确要求时处理 fuzz 回归或崩溃复现。接口选型/所有权咨询转 ryanjson-api-usage，内部优化转 ryanjson-optimization。
---
...
## 去重检查清单
- 在目标目录与全树 `rg` 搜索 API/字面量，确认无同语义覆盖。
...
- `test/unityTest/runner/test_list.inc` 由 `scripts/ci/checkUnityRunnerList.sh` 自动生成，禁止手改；新增/删除 runner 后运行脚本或 `./run_local_base.sh`。
