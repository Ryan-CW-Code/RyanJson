...
{
	// 覆盖 key 路径在中途遇到数组节点时应立即停止：
	// 1) GetObjectByKeys 返回 NULL；
	// 2) HasObjectToKey 返回 false；
	// 3) 邻近合法路径不受影响。
	...
	TEST_ASSERT_NULL_MESSAGE(RyanJsonGetObjectByKeys(root, "cfg", "list", "id", NULL), "对象 key 路径在数组节点处应停止");
	TEST_ASSERT_FALSE_MESSAGE(RyanJsonHasObjectToKey(root, "cfg", "list", "id"), "HasObjectToKey 在数组节点处应为 false");
...
{
	// 覆盖 index 路径可穿过对象容器，但在最终落到标量叶子后必须停止。
	...
	TEST_ASSERT_NULL_MESSAGE(RyanJsonGetObjectByIndexs(root, 0, 0, 0, 0, UINT32_MAX), "数组 index 路径在标量叶子后应停止");
...
static void testAccessorPathGuardMismatchDoesNotPoisonSubsequentLookups(void)
{
	...
	TEST_ASSERT_NULL_MESSAGE(RyanJsonGetObjectByKeys(root, "left", "name", NULL), "先经过数组再取 key 应失败");
	TEST_ASSERT_NULL_MESSAGE(RyanJsonGetObjectByIndexs(RyanJsonGetObjectByKey(root, "right"), 0, 0, 0, UINT32_MAX),
				 "先经过对象容器再落到标量后继续取 index 应失败");
